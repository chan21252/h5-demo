<template>
  ...
  >
    <div
      ...
      class="card-item"
    >
      <div class="item-inner" :id="'item-' + index" @click="go(index)"></div>
    </div>
  </div>
</template>
        ...
        if (this.disX < 0) {
          ...
          if (this.cardArrs[this.currentIndex - 1]) {
            ...
            if (-this.disX <= this.slideFinishDistance) {
              item_0.scale =
                -(0.2 / this.slideFinishDistance) * this.disX + 0.8;
            }
          }
          // 当前第一个变化
          ...
          if (-this.disX <= this.slideFinishDistance) {
            item_1.translateX =
              -(9 / this.slideFinishDistance) * this.disX + "vw";
          ...
          if (this.cardArrs[this.currentIndex + 1]) {
            ...
            if (-this.disX <= this.slideFinishDistance) {
              item_2.translateX =
                -(6 / this.slideFinishDistance) * this.disX + 9 + "vw";
              item_2.transitionTime = 0;
              item_2.scale =
                (0.05 / this.slideFinishDistance) * this.disX + 0.9;
            }
          }
          // 当前第三个变化
          if (this.cardArrs[this.currentIndex + 2]) {
            ...
            if (-this.disX <= this.slideFinishDistance) {
              item_3.translateX =
                -(25 / this.slideFinishDistance) * this.disX + 15 + "vw";
              item_3.transitionTime = 0;
              item_3.scale =
                (0.35 / this.slideFinishDistance) * this.disX + 0.85;
            }
          }
        } else if (this.disX > 0) {
          //左滑
          ...
          if (this.cardArrs[this.currentIndex + 1]) {
            ...
            if (this.disX <= this.slideFinishDistance) {
              item_2.translateX =
                -(9 / this.slideFinishDistance) * this.disX + 9 + "vw";
              item_2.transitionTime = 0;
          ...
          if (this.cardArrs[this.currentIndex + 2]) {
            ...
            if (this.disX <= this.slideFinishDistance) {
              item_3.translateX =
                -(6 / this.slideFinishDistance) * this.disX + 15 + "vw";
              item_3.transitionTime = 0;
              item_3.scale =
                (0.05 / this.slideFinishDistance) * this.disX + 0.85;
            }
          }
          // 当前第四个变化
          if (this.cardArrs[this.currentIndex + 3]) {
            ...
            if (this.disX <= this.slideFinishDistance) {
              item_4.translateX =
                -(25 / this.slideFinishDistance) * this.disX + 40 + "vw";
              item_4.transitionTime = 0;
              item_4.scale =
                (0.35 / this.slideFinishDistance) * this.disX + 0.5;
            }
          }
        }
      }
      ...
      try {
        if (isIOS) {
          try {
            ...
            window.webkit.messageHandlers.thepaperNewsClicked.postMessage(
              this.appLinks[id]
            );
          }
        } else if (isAndroid) {
...
.item-inner {
  ...
  height: 100%;
  margin: 0 auto;
  box-shadow: 0 4px 12px 1px rgba(57, 57, 57, 0.2);
  position: relative;
  border-radius: 20px;
